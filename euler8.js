...
const getArrayFromString = (digitsString = "") =>
  digitsString
    .replace(/(\n)/gm, "")
    .split("")
    .map((s) => parseInt(s, 10));

const getAdjacentChunks = (digits = [], chunkSize = 4) =>
  digits
    .map((n, key) => {
      const chunk = [];
      for (let i = 0; i < chunkSize; i += 1) {
        chunk.push(digits[key + i]);
      }
...
const getAdjacentProductsRanked = (
  ...
  const digits = getArrayFromString(digitsString) || [];
  ...
  return adjacentChunkProducts
    .sort((a, b) => a - b)
    .slice(adjacentChunks.length - count, adjacentChunks.length)
    .reverse();
};

console.log(getAdjacentProductsRanked(hugeNumber, 13, 1));
